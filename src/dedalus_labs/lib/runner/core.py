...
def _process_policy(
    policy: PolicyInput, context: PolicyContext
) -> Dict[str, JsonValue]:
...
class DedalusRunner:
    ...
    def run(
        ...
        if isinstance(model, list):
                ...
                if hasattr(m, 'name'):  # DedalusModel
                    ...
                    if model_name is None:
                        ...
                        temperature = (
                            ...
                            else getattr(m, 'temperature', None)
                        )
                        max_tokens = (
                            max_tokens
                            if max_tokens is not None
                            else getattr(m, 'max_tokens', None)
                        )
                        top_p = (
                            top_p if top_p is not None else getattr(m, 'top_p', None)
                        )
                        frequency_penalty = (
                            frequency_penalty
                        ...
                        presence_penalty = (
                            ...
                            else getattr(m, 'presence_penalty', None)
                        )
                        logit_bias = (
                            logit_bias
                            if logit_bias is not None
                            else getattr(m, 'logit_bias', None)
                        )
                        ...
                        stream = (
                            ...
                            else getattr(m, 'stream', False)
                        )
                        tool_choice = (
                            tool_choice
                            if tool_choice is not None
                            else getattr(m, 'tool_choice', None)
                        )
                        ...
                        if (
                            hasattr(m, 'stream_options')
                            and m.stream_options is not None
                        ):
                        ...
                        if (
                            hasattr(m, 'parallel_tool_calls')
                            and m.parallel_tool_calls is not None
                        ):
                            unsupported_params.append('parallel_tool_calls')
                        if hasattr(m, 'user') and m.user is not None:
                            unsupported_params.append('user')
                        if (
                            hasattr(m, 'max_completion_tokens')
                            and m.max_completion_tokens is not None
                        ):
            ...
            temperature = (
                temperature
                if temperature is not None
                else getattr(model, 'temperature', None)
            )
            max_tokens = (
                max_tokens
                if max_tokens is not None
                else getattr(model, 'max_tokens', None)
            )
            top_p = top_p if top_p is not None else getattr(model, 'top_p', None)
            frequency_penalty = (
                frequency_penalty
                if frequency_penalty is not None
                else getattr(model, 'frequency_penalty', None)
            )
            presence_penalty = (
                presence_penalty
                if presence_penalty is not None
                else getattr(model, 'presence_penalty', None)
            )
            logit_bias = (
                logit_bias
                if logit_bias is not None
                else getattr(model, 'logit_bias', None)
            )
            ...
            tool_choice = (
                tool_choice
                if tool_choice is not None
                else getattr(model, 'tool_choice', None)
            )
            ...
            if hasattr(model, 'tools') and model.tools is not None:
                unsupported_params.append('tools')
            if (
                hasattr(model, 'parallel_tool_calls')
                and model.parallel_tool_calls is not None
            ):
                unsupported_params.append('parallel_tool_calls')
        ...
        if instructions is not None and messages is not None:
            ...
            if input is not None:
                if isinstance(input, str):
                    ...
                    conversation = [{'role': 'system', 'content': instructions}] + list(
                        input
                    )
            else:
                conversation = [{'role': 'system', 'content': instructions}]
            ...
            raise ValueError(
                "Must provide one of: 'instructions', 'messages', or 'input'"
            )

        return self._execute_conversation(
            conversation, tool_handler, model_config, exec_config
        )
    ...
    def _execute_conversation(
        ...
        if is_async:
            if exec_config.stream:
                return self._execute_streaming_async(
                    messages, tool_handler, model_config, exec_config
                )
            else:
                return self._execute_turns_async(
                    messages, tool_handler, model_config, exec_config
                )
        else:
            if exec_config.stream:
                return self._execute_streaming_sync(
                    messages, tool_handler, model_config, exec_config
                )
            else:
                return self._execute_turns_sync(
                    messages, tool_handler, model_config, exec_config
                )
    ...
    async def _execute_turns_async(
        ...
        while steps < exec_config.max_steps:
            ...
            current_messages = self._build_messages(
                messages, policy_result['prepend'], policy_result['append']
            )
            ...
            if exec_config.verbose:
                ...
                print(
                    f'  Response received (server says model: {getattr(response, "model", "unknown")})'
                )
            ...
            if exec_config.verbose:
                ...
                for tc in tool_calls:
                    print(
                        f'  - {tc.get("function", {}).get("name", "?")} (id: {tc.get("id", "?")})'
                    )
    ...
    async def _execute_streaming_async(
        ...
        while steps < exec_config.max_steps:
            ...
            if exec_config.verbose:
                ...
                print(
                    f' Starting step {steps} with {len(messages)} messages in conversation'
                )
            ...
            async for chunk in stream:
                ...
                if exec_config.verbose:
                    ...
                    if isinstance(extra, dict):
                        meta = extra.get('x_dedalus_event') or extra.get(
                            'dedalus_event'
                        )
                        if (
                            isinstance(meta, dict)
                            and meta.get('type') == 'agent_updated'
                        ):
                            print(
                                f' [EVENT] agent_updated: agent={meta.get("agent")} model={meta.get("model")}'
                            )
            ...
            if exec_config.verbose:
                ...
                print(
                    f' Stream summary: chunks={chunk_count} content={content_chunks} tool_calls={tool_call_chunks}'
                )
                if names:
            ...
            if tool_calls:
                ...
                if mcp_names and has_streamed_content:
                    if exec_config.verbose:
                        print(
                            f' MCP tools called and content streamed - response complete, breaking loop'
                        )
                    break
                ...
                if all_mcp:
                    ...
                    messages.append(
                        {'role': 'assistant', 'tool_calls': local_only_tool_calls}
                    )
                    if exec_config.verbose:
                        print(
                    ...
                    for tc in tool_calls:
                        ...
                        if fn_name in getattr(tool_handler, '_funcs', {}):
                            ...
                            try:
                                ...
                                if exec_config.verbose:
                                    print(
                                        f' Executed local tool {fn_name}: {str(result)[:50]}...'
                                    )
                            ...
                            if exec_config.verbose:
                                print(
                                    f' MCP tool {fn_name} - skipping (server will handle)'
                                )
                    ...
                    if exec_config.verbose:
                        ...
                        if not mcp_names:
                            print(
                                f' No MCP tools, continuing loop to step {steps + 1}...'
                            )
                        else:
                            print(
                                f' MCP tools present, expecting response in next iteration'
                            )
    ...
    def _execute_turns_sync(
        ...
        while steps < exec_config.max_steps:
            ...
            if exec_config.verbose:
                print(
                    f'  Response content: {content[:100] if content else "(none)"}...'
                )
                if tool_calls:
                    tool_names = [
        ...
        while steps < exec_config.max_steps:
            ...
            if exec_config.verbose:
                ...
                for i, msg in enumerate(current_messages):
                    ...
                    print(
                        f'  [{i}] {msg.get("role")}: {content_preview}...{tool_call_info}'
                    )
                print(f' MCP servers: {policy_result["mcp_servers"]}')
                print(
                    f' Local tools available: {list(getattr(tool_handler, "_funcs", {}).keys())}'
                )
            ...
            if exec_config.verbose:
                ...
                if tool_calls:
                    ...
                    for i, tc in enumerate(tool_calls, 1):
                        ...
                        display_name = (
                            tool_name.replace('transfer_to_', '')
                            .replace('_', ' ')
                            .title()
                        )
    ...
    def _apply_policy(
        ...
        if pol:
            ...
            if (
                requested_model
                and exec_config.strict_models
                and exec_config.available_models
            ):
                if isinstance(requested_model, list):
                    # Filter to only available models
                    valid_models = [
                        m for m in requested_model if m in exec_config.available_models
                    ]
                    if valid_models:
                        ...
                        print(
                            f'[RUNNER] Policy requested unavailable models {requested_model}, ignoring'
                        )
                elif requested_model not in exec_config.available_models:
                    if exec_config.verbose:
                        print(
                            f"[RUNNER] Policy requested unavailable model '{requested_model}', ignoring"
                        )
                else:
    ...
    async def _execute_tool_calls(
        ...
        for i, tc in enumerate(tool_calls):
            ...
            try:
                ...
                if verbose:
                    print(
                        f' Tool {fn_name} executed successfully: {str(result)[:50]}...'
                    )
            except Exception as e:
